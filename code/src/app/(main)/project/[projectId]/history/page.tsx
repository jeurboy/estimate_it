...
const HistoryPage = () => {
    ...
    const subTaskColumns = [
        ...
        { title: 'Days', dataIndex: 'Days', key: 'days', width: '10%', editable: true, render: (days: number | string) => parseFloat(String(days || '0')).toFixed(2) },
        {
            ...
            dataIndex: 'action',
            width: '15%',
            render: (_: unknown, record: EditableSubTask & { key: React.Key }) => {
    ...
    const editSubTask = (record: EditableSubTask & { key: React.Key }) => {
    ...
    const addNewSubTask = () => {
        ...
        const newSubTask: EditableSubTask & { key: React.Key } = { 'Sub-Task': newKey, Description: '', Days: 0, key: newKey };
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <Title level={2} style={{ margin: 0 }}>
                                Task: {selectedProject ? selectedProject.name_en : 'No Project Selected'}
                            </Title>
                        </div>
                        <Button
                            onClick={() => handleExportCSV(nonReferenceHistory.filter(h => h.project_id === selectedProject?.id))}
                            disabled={nonReferenceHistory.filter(h => h.project_id === selectedProject?.id).length === 0}
                        >
                            ส่งออกเป็น CSV
                        </Button>
                    </div>
                ...
                <Card>
                    <Spin spinning={loading} tip="Loading tasks...">
                        <Table
                            ...
                            locale={{
                                emptyText: selectedProject ? 'No tasks found for this project.' : 'Please select a project to view its tasks.'
                            }}
                        />
                    </Spin>
                </Card>
                        ...
                        <Form form={subTaskForm} component={false}>
                            <Table
                                ...
                                columns={subTaskColumns.map(col => {
                                    ...
                                    return {
                                        ...col,
                                        onCell: (record: EditableSubTask & { key: React.Key }): object => ({
                                            record,
                                ...
                                dataSource={selectedRecord.sub_tasks.map((task: EditableSubTask, index) => ({ ...task, key: task.key || task['Sub-Task'] || `task-${index}` }))}
                                rowKey="key"
                                pagination={false}
                                size="small"
                                bordered
